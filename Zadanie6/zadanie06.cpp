...
public:
    ...
    char getChar(size_t index) const{
        if(index < strlen(data)) return data[index];
        else return '\0';
    }
    //f
    ...
    void append(const char *text){
        if(text){
            size_t original = strlen(data);
            size_t append = strlen(text);

            char* newData = new char[original + append + 1];
            strcpy(newData, data);
            strcpy(newData + original, text);
