...
void replace200to400by5(list<int> & data) noexcept {
    return replace_if(data.begin(), data.end(), isInRange200to400, 5);
}
...
void helpAfterExam2(list<int> & points) noexcept {
    for(auto& i : points) i = incrementPointsIfItHelps(i);
}
...
int main() {
    ...
    /*
    try {
        initializer_list<int> list1 = {5, 7, 4, 2};
        cout << "Priemer: " << average(list1) << endl;

        initializer_list<int> list2 = {};
        cout << "Priemer: " << average(list2) << endl;

        initializer_list<int> list3 = {5, 3, 4, 4};
        cout << "Priemer: " << average(list3) << endl;
    } catch (const invalid_argument& e) {
        cerr << "Výnimka: " << e.what() << endl;
    }
     */

    //2
    /*
    list<int> data = {2, 5, 1, 4, 3, 5};

    auto it = findValue(data, 6);
    if (it != data.cend()) {
        cout << "Hodnota nájdená: " << *it << endl;
    } else {
        cout << "Hodnota nebola nájdená." << endl;
    }
    */
