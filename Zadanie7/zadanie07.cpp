...
queue<int> reverse(const list<int> & data) noexcept {
    ...
    for (int it : std::ranges::reverse_view(data)) {
        new_queue.push(it);
    }
...
void reverse(queue<int> & data) noexcept {
    ...
    while(!data.empty()){
        new_stack.push(data.front());
        data.pop();
    }

    while(!new_stack.empty()){
        data.push(new_stack.top());
        new_stack.pop();
    }
}
...
vector<vector<int>> create2dVector(const size_t size1, const size_t size2, const int value) noexcept {
    vector<int> inner_vector(size2, value);
    vector<vector<int>> new_vector_all(size1, inner_vector);

    return new_vector_all;
}
...
list<string> translate(const map<string, string> & translator, const list<string> & sentence) noexcept {
    ...
    for(const auto& word : sentence){
        ...
        if(it != translator.end()) translated_sentence.push_back(it->second);
        else translated_sentence.emplace_back("?");
    }
...
bool areParenthesisPaired(const string & mathExpression) noexcept {
    ...
    std::map<char, char> matchingParenthesis = {
            {')', '('},
            {'}', '{'},
            {']', '['},
            {'>', '<'}
    ...
    for(char ch : mathExpression){
        if(ch == '(' || ch == '{' || ch == '[' || ch == '<') new_stack.push(ch);
        else if (ch == ')' || ch == '}' || ch == ']' || ch == '>'){
            if(new_stack.empty() || new_stack.top() != matchingParenthesis[ch])
                return false;
...
int main() {
    ...
    */
    ...
    /*
    vector<vector<int>> vector = create2dVector(2, 5, 10);
    for (const auto& inner_vec : vector) {
    ...
    */
    ...
    //4
    /*
    string result = connectWithSpace("abc", 10);
    cout << "Výstup: \"" << result << "\"" << std::endl;
    */

    //5
    /*
    istringstream input1("10 20 30 10");
    cout << "Output: " << sum(input1) << endl;
    */

    //6
    //7
    /*
    string expression = "a + (2 * (3 + 4)))";
    cout << (areParenthesisPaired(expression) ? "true" : "false") << std::endl;
    */
