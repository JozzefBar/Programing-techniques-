...
void siftUp(int data[], const size_t addIndex)
{
    ...
    if(data[addIndex] < data[i_parent]){
        swap(&data[i_parent], &data[addIndex]);
...
{
    size_t left = 2*topIndex + 1;
    size_t right = 2*topIndex + 2;
    size_t iswap = topIndex;

    if(left >= length) {
        return;
    }
    if (right < length && data[right] < data[topIndex]) iswap = right;
    if (data[left] < data[iswap]) iswap = left;

    if(iswap != topIndex){
        swap(&data[iswap], &data[topIndex]);
...
int main() {
    ...
    //úloha 2
    //buildHeapSiftUp(data, len);

    //úloha 3
    //siftDown(data, 1, len);

    //úloha 4
    //buildHeapSiftDown(data, len);
